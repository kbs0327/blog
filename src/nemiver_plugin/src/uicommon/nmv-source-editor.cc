...
gboolean on_button_press(GtkWidget * widget, GdkEventButton * event, GdkWindowEdge edge)
 {
     ...
     func_mngr->onClicked(event->x, event->y);
     return FALSE;
 }
void
...
MyArea::MyArea()
{
    path = "diagram";
	m_select=1;
    gtk_widget_add_events(((Gtk::Widget*)this)->gobj (), GDK_BUTTON_PRESS_MASK);
    g_signal_connect (((Gtk::Widget*)this)->gobj (),
                      "button-press-event",
                      G_CALLBACK (&on_button_press),
                      this);
}

MyArea::MyArea(Gtk::Widget *w) : parent(w)
{
    path = "diagram";
    m_select=1;
    gtk_widget_add_events(((Gtk::Widget*)this)->gobj (), GDK_BUTTON_PRESS_MASK);

      g_signal_connect(((Gtk::Widget*)this)->gobj (), "button-press-event",
          G_CALLBACK(&on_button_press), this);

    /*gtk_widget_add_events(((GtkWidget*)gobj ()), GDK_BUTTON_PRESS_MASK);
    g_signal_connect (gobj(),
                      "button-press-event",
                      G_CALLBACK (&MyArea::on_button_press),
                      this);*/
    //g_signal_connect(G_OBJECT(this), "button-press-event", G_CALLBACK(&MyArea::on_button_press), NULL);
}
MyArea::MyArea(int select)
{
	path = "diagram";
    m_select=2;
    gtk_widget_add_events(((Gtk::Widget*)this)->gobj (), GDK_BUTTON_PRESS_MASK);
...
void
MyArea::get_path (UString& a_path) const
{
    a_path = path;
}
...
bool MyArea::on_draw(const Cairo::RefPtr<Cairo::Context>& cr)
{
  ...
  yc = height;
   ...
   // outline thickness changes
                                      // with window size

  MyFunction::setCairo(cr);
  MyFunction::setWidget(this);
  MyFunctionManager* func_mngr = MyFunctionManager::getFuncManager();
  THROW_IF_FAIL(func_mngr);
  func_mngr->draw(width, height);
  set_size_request(func_mngr->getWinX(), func_mngr->getWinY());

  //Prgoress_MyFunctionManager* progress_func_mngr = Prgoress_MyFunctionManager::getFuncManager();
 // THROW_IF_FAIL(func_mngr);
  //progress_func_mngr->draw(width, height);
  ...
  else if(m_select==2)
  {
      ...
      MyFunction::setWidget(this);
      Prgoress_MyFunctionManager* progress_func_mngr = Prgoress_MyFunctionManager::getFuncManager();
      THROW_IF_FAIL(progress_func_mngr);
      progress_func_mngr->draw(width, height);
      set_size_request(progress_func_mngr->getWinX(), progress_func_mngr->getWinY());
