...
private:
    ...
    //changed...
    void on_activate_local_variables ();
    ...
    void append_source_editor2 (SourceEditor &a_sv,
                               const UString &a_path);
    void append_diagram();
    void run_progressbar();
...
public:
    ...
    Gtk::Widget& get_source_view_widget2 ();
  //  Gtk::Widget& get_progressbar_widget ();
    ...
    Gtk::Widget& get_terminal_box ();
    Gtk::Widget& get_call_stack_box ();
...
struct DBGPerspective::Priv {
    ...
    SafePtr<Gtk::Notebook> sourceviews_notebook2;
    ...
    map<int, Gtk::Widget*> pagenum_2_source_editor_map;
    ...
    SafePtr<Gtk::Box> call_stack_box;
    SafePtr<Gtk::Box> terminal_box;
    SafePtr<Gtk::Box> progressbar;
    ...
    modify_source_editor_style (Glib::RefPtr<Gsv::StyleScheme> a_style_scheme)
    {
        ...
        map<int, Gtk::Widget*>::iterator it;
        for (it = pagenum_2_source_editor_map.begin ();
                it != pagenum_2_source_editor_map.end ();
                ++it) {
            if(it->second == dia_adr)
                continue;
            if (it->second) {
                ((SourceEditor*)it->second)->source_view ().get_source_buffer ()->set_style_scheme (a_style_scheme);
            }
        }
    }
    ...
    modify_source_editor_fonts (const UString &a_font_name)
    {
        ...
        for (it = pagenum_2_source_editor_map.begin ();
            ...
            if(it->second == dia_adr)
                continue;
            if (it->second) {
               ((SourceEditor*)it->second)->source_view ().override_font (font_desc);
            }
        }
...
DBGPerspective::on_step_into_action ()
{
    ...
    MyFunctionManager* func_mngr = MyFunctionManager::getFuncManager();
    if(!func_mngr->isDrawing())
    {
         func_mngr->restartWithDrawingToTarget();
    }
                                       ...
                                       guint a_page_num)
{
    ...
    if(a_page_num == 0)
    {
        if(m_priv->pagenum_2_source_editor_map[a_page_num])
         ((Gtk::DrawingArea*)(((Gtk::ScrolledWindow*)m_priv->pagenum_2_source_editor_map[a_page_num])->get_child()))->queue_draw ();
    }
                                            ...
                                            const UString &a_namespace)
{
    ...
    if (a_key == CONF_KEY_NEMIVER_SOURCE_DIRS) {
        ...
        for (it = m_priv->pagenum_2_source_editor_map.begin ();
             ...
             ++it) {
            if(it->second == m_priv->dia_adr)
                continue;
            if (it->second) {
               ((SourceEditor*)it->second)->source_view ().set_show_line_numbers
                    (show_line_numbers);
            }
        }
        ...
        for (it = m_priv->pagenum_2_source_editor_map.begin ();
            ...
            if(it->second == m_priv->dia_adr)
                continue;
            if (it->second && ((SourceEditor*)it->second)->source_view ().get_buffer ()) {
                ((SourceEditor*)it->second)->source_view ().get_source_buffer
...
DBGPerspective::on_debugger_running_signal ()
{
    ...
    //changed.. its cfeate partialcode..
    disassemble(true);
    NEMIVER_CATCH
}
...
//changed..
void
DBGPerspective::on_activate_local_variables ()
{
    LOG_FUNCTION_SCOPE_NORMAL_DD;
    ...
    get_local_vars_inspector().dialogrun(plugin_path (),
                                         debugger (),
                                         workbench ());

    NEMIVER_CATCH
    //variables_editor.run();
...
DBGPerspective::init_actions ()
{
    ...
    static ui_utils::ActionEntry s_default_action_entries [] = {
        ...
        {
            "FileInMenuItemAction",
            Gtk::Stock::OPEN,
            _("FILE"),
            _("Load a executable file from disk"),
            sigc::mem_fun (*this,
                           &DBGPerspective::on_execute_program_action),
            ActionEntry::DEFAULT,
            "",
    ...
    MyFunctionManager* func_mngr = MyFunctionManager::getFuncManager();
    //func_mngr->setDBGPersp(*this);
    func_mngr->getRestartSignal().connect(sigc::mem_fun
                                          (this, &DBGPerspective::on_run_action));
}
...
DBGPerspective::init_body ()
{
 ...
 //   get_context_paned ().pack1 (get_local_vars_inspector_scrolled_win ());
...
//    int context_pane_location = -1;
 ...
 //       get_context_paned ().set_position (context_pane_location);
  ...
  //                                 (get_local_vars_inspector ().widget ());
    ...
    m_priv->sourceviews_notebook2.reset (new Gtk::Notebook);
    m_priv->sourceviews_notebook2->remove_page ();
    m_priv->sourceviews_notebook2->set_show_tabs (false);
    m_priv->sourceviews_notebook2->set_scrollable ();
    m_priv->sourceviews_notebook2->set_size_request (250, 200);

#if GTK_CHECK_VERSION (2, 10, 0)
    m_priv->sourceviews_notebook2->signal_page_reordered ().connect
        (sigc::mem_fun (this, &DBGPerspective::on_notebook_tabs_reordered));
#endif
...
DBGPerspective::init_signals ()
{
    ...
    MyFunctionManager::getFuncManager()->setDBGPersp(*this);
...
void DBGPerspective::run_progressbar()
{
   ...
   color.blue = 0.33;
    ...
    LOG_FUNCTION_SCOPE_NORMAL_DD;
   Gtk::Box *progressbar = workbench ().get_progressbar();
  // Gtk::GtkWidget *progressbar = workbench ().get_progressbar();
  // gtk_widget_override_background_color (progressbar, GTK_STATE_FLAG_NORMAL, &color);

   scrwin = new Gtk::ScrolledWindow();
   scrwin->set_border_width(10);
   scrwin->set_policy(Gtk::POLICY_ALWAYS, Gtk::POLICY_ALWAYS);
   scrwin->show();

   MyArea *area = new MyArea(2);
   scrwin->add(*area);
   area->show ();

   progressbar->pack_start (*scrwin);
   progressbar->show_all ();

}
void DBGPerspective::append_diagram()
{
    UString dia_path = "diagram";
    if (m_priv->path_2_pagenum_map.find (dia_path)
        != m_priv->path_2_pagenum_map.end ()) {
       return;
    }

    SafePtr<Gtk::Label> label (Gtk::manage
                            (new Gtk::Label (dia_path)));
    label->set_ellipsize (Pango::ELLIPSIZE_MIDDLE);
    label->set_width_chars (dia_path.length ());
    ...
    close_button->file_path = dia_path;
    close_button->signal_clicked ().connect
            (sigc::mem_fun (*close_button, &SlotedButton::on_clicked));
    UString message;
    message.printf (_("Close %s"), dia_path.c_str ());
    ...
    event_box->set_tooltip_text (dia_path);
    hbox->show_all ();
    Gtk::ScrolledWindow * scroll_win = new Gtk::ScrolledWindow();
    scroll_win->set_border_width(10);
    scroll_win->set_policy(Gtk::POLICY_ALWAYS, Gtk::POLICY_ALWAYS);
    scroll_win->show();

    MyArea* area = new MyArea(scroll_win);

    scroll_win->add(*area);
    area->show ();

    int page_num = m_priv->sourceviews_notebook->insert_page (*scroll_win,
                                                              *hbox,
                                                              -1);
    m_priv->sourceviews_notebook->set_tab_reorderable (*scroll_win);

    std::string base_name =
                Glib::path_get_basename (Glib::filename_from_utf8 (dia_path));
    THROW_IF_FAIL (base_name != "");
    m_priv->basename_2_pagenum_map[Glib::filename_to_utf8 (base_name)] =
                                                                    page_num;
    m_priv->path_2_pagenum_map[dia_path] = page_num;
    m_priv->pagenum_2_source_editor_map[page_num] = scroll_win;
    m_priv->pagenum_2_path_map[page_num] = dia_path;
    m_priv->dia_adr = scroll_win;
...
void
DBGPerspective::append_source_editor2 (SourceEditor &a_sv,
                                      const UString &a_path)
{
    UString path = a_path;
    if (path.empty ()) {
    ...
    hbox->show_all ();
    a_sv.show_all ();
    // change.. assemble page_num ==200;
    int page_num=200;
    m_priv->sourceviews_notebook2->insert_page (a_sv,
                                                              *hbox,
                                                              -1);
    m_priv->sourceviews_notebook2->set_tab_reorderable (a_sv);
                                             ...
                                             bool a_basename_only)
{
    ...
    result = ((SourceEditor*)m_priv->pagenum_2_source_editor_map[iter->second]);
    if(result == m_priv->dia_adr)
        return 0;
...
DBGPerspective::get_or_append_asm_source_editor ()
{
    ...
    if (source_editor == 0) {
        ...
        append_source_editor2 (*source_editor, get_asm_title ());
    }
    THROW_IF_FAIL (source_editor);
    return source_editor;
}
...
{
    map<int, Gtk::Widget*>::iterator iter;
    for (iter = m_priv->pagenum_2_source_editor_map.begin ();
         iter !=m_priv->pagenum_2_source_editor_map.end ();
         ++iter) {
        if(iter->second == m_priv->dia_adr)
            continue;
        if (!(iter->second)) {continue;}
        ((SourceEditor*)iter->second)->unset_where_marker ();
    }
}
...
Gtk::Widget&
DBGPerspective::get_source_view_widget2 ()
{
    return *m_priv->sourceviews_notebook2;
}
/*
Gtk::Widget&
DBGPerspective::get_progressbar_widget ()
{

    THROW_IF_FAIL (m_priv);
    if (!m_priv->progressbar) {
        m_priv->progressbar.reset (new Gtk::HBox);
        Gtk::ScrolledWindow * scroll_win = new Gtk::ScrolledWindow();
        scroll_win->set_border_width(10);
        scroll_win->set_policy(Gtk::POLICY_ALWAYS, Gtk::POLICY_ALWAYS);
        scroll_win->show();
        ...
        m_priv->progressbar->pack_start (*scroll_win);
    }
    THROW_IF_FAIL (m_priv->progressbar);

    return *m_priv->progressbar;
}
...
DBGPerspective::close_file (const UString &a_path)
{
    ...
    Prgoress_MyFunctionManager* func_mngr = Prgoress_MyFunctionManager::getFuncManager();
    func_mngr->clear();

    Gtk::Container *progressbar = workbench ().get_progressbar();
    progressbar->remove(*scrwin);
    ...
    {
        m_priv->sourceviews_notebook2->remove_page (0);
    }
...
DBGPerspective::update_file_maps ()
{
    ...
    for (int i = 0; i < nb_pages; ++i) {
        se = dynamic_cast<Gtk::Widget*>
            (m_priv->sourceviews_notebook->get_nth_page (i));
        THROW_IF_FAIL (se);
        if(se == m_priv->dia_adr)
            ((MyArea*)se)->get_path(path);
        else
            ((SourceEditor*)se)->get_path (path);
...
DBGPerspective::restart_inferior ()
{
    ...
    if (!is_connected_to_remote_target ()) {
        ...
        // We cannot restart an inferior ning on a remote target at
        // the moment.
...
/// function just re-s it. The advantage of doing this is that the
...
DBGPerspective::restart_local_inferior ()
{
    ...
    if (// If the program the user asked is a libtool wrapper shell
        ...
        // to re- the binary it's current ning, as libtool can
        ...
        // engine to ask a re- to ...
        && debugger ()->is_attached_to_target ()
        // Make sure we are restarting the program we were ning
...
//change..
void
DBGPerspective::localvariable_dialog_run()
{
}
...
Gtk::Widget &
DBGPerspective::get_call_stack_box ()
{
    THROW_IF_FAIL (m_priv);
    if (!m_priv->call_stack_box) {
        m_priv->call_stack_box.reset (new Gtk::HBox);
        THROW_IF_FAIL (m_priv->call_stack_box);
        Gtk::VScrollbar *scrollbar = Gtk::manage (new Gtk::VScrollbar);
        m_priv->call_stack_box->pack_end (*scrollbar, false, false, 0);
        m_priv->call_stack_box->pack_start (get_call_stack().widget (),Gtk::PACK_EXPAND_WIDGET);
//        scrollbar->set_adjustment (get_call_stack().adjustment ());
    }
    THROW_IF_FAIL (m_priv->call_stack_box);

    return *m_priv->call_stack_box;
}
