...
public:
	...
	//changed..
    bool on_dialog;
	GlobalVarsInspectorDialog *dialog;
...
/*
   //changed..
    class NEMIVER_API GlobalVarsInspectorDialog2 : public Dialog {
        //non copyable
		friend struct LocalVarsInspector::Priv;
        GlobalVarsInspectorDialog2 (const GlobalVarsInspectorDialog2&);
        GlobalVarsInspectorDialog2& operator= (const GlobalVarsInspectorDialog2&);

		
    protected:
        GlobalVarsInspectorDialog2 ();

    public:
		  
        GlobalVarsInspectorDialog2
            (const UString &a_root_path,
                IDebuggerSafePtr &a_debugger, IWorkbench &a_workbench, Gtk::Widget &tree_view) :
            Dialog (a_root_path,
                    ...
                    "globalvarsinspector")
        {
            //changed..;
                  ...
                  close_button->signal_clicked ().connect
                     (sigc::mem_fun (*this, &GlobalVarsInspectorDialog2::on_close_button_clicked_signal));
...
//
 //       }
		...
		static GlobalVarsInspectorDialog2 *getDialog()
		{
			static GlobalVarsInspectorDialog2 dialog;
			return &dialog;
		}
    ...
    };//end GlobalVarsInspectorDialog2
...
*/
    ...
    append_a_local_variable (const IDebugger::VariableSafePtr a_var)
    {
        ...
        if (get_local_variables_row_iterator (parent_row_it)) {
            ...
            //MyFunction extention
            MyFunctionManager * func_mngr = MyFunctionManager::getFuncManager();
            if(saved_has_frame)
            {
                func_mngr->setAddable();
                UString var_name = a_var->name();
                var_name.chomp();
                func_mngr->addLocalVal(var_name, a_var->value(), a_var->type());
            }
        }
    }
    ...
    append_a_function_argument (const IDebugger::VariableSafePtr a_var)
    {
        ...
        if (get_function_arguments_row_iterator (parent_row_it)) {
            ...
            if(saved_has_frame)
            {
                ...
                UString var_name = a_var->name();
                var_name.chomp();
                func_mngr->addArg(var_name, a_var->value(), a_var->type());
            }
        }
    }
                             ...
                             bool a_update_members = true)
    {
        ...
        if (get_local_variables_row_iterator (parent_row_it)) {
            vutil::update_a_variable (a_var, *tree_view,
                                      ...
                                      a_update_members);
            MyFunctionManager * func_mngr = MyFunctionManager::getFuncManager();
                func_mngr->setEditable();
                UString var_name = a_var->name();
                var_name.chomp();
                func_mngr->editLocalVal(var_name, a_var->value(), a_var->type());
        }
    }
    ...
    update_a_visualized_local_variable (const IDebugger::VariableSafePtr a_var)
    {
        ...
        if (get_local_variables_row_iterator (parent_row_it)) {
            THROW_IF_FAIL (vutil::find_a_variable (a_var,
                                                   parent_row_it,
                                                   row_it));
            //추가 해야됨 -변수 지우기나 값 변경하기
            MyArea* area  = (MyArea *)MyFunction::getWidget();
            area->show();
    ...
    update_a_function_argument (const IDebugger::VariableSafePtr a_var)
    {
        ...
        if (get_function_arguments_row_iterator (parent_row_it)) {
            ...
            UString var_name = a_var->name();
            var_name.chomp();
            func_mngr->editArg(var_name, a_var->value(), a_var->type());
                       ...
                       const UString &/*a_cookie*/)
    {
        ...
        MyFunctionManager* func_mngr = MyFunctionManager::getFuncManager();
        func_mngr->setFuncName(saved_frame.function_name(), is_new_frame,
                               saved_frame.file_full_name(), saved_frame.file_name (), saved_frame.line());
	...
	void dialogrun2(const UString &a_root_path,
               IDebuggerSafePtr &a_dbg,
               IWorkbench &a_wb, Gtk::Widget &tree_view)
	{

        if(on_dialog==false)
        {
			dialog=new GlobalVarsInspectorDialog(a_root_path, a_dbg, a_wb, tree_view);
            on_dialog==true;
            dialog->run();
        }
        else
...
//changed..
void LocalVarsInspector::dialogrun(const UString &a_root_path,
               IDebuggerSafePtr &a_dbg,
               IWorkbench &a_wb)
{

	THROW_IF_FAIL (m_priv);
    m_priv->dialogrun2(a_root_path, a_dbg, a_wb, widget ());
