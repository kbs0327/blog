...
#ifndef GTKMM_EXAMPLE_MYAREA2_H
#define GTKMM_EXAMPLE_MYAREA2_H
...
private:
    ...
    void init_toolbar ();
    void init_body ();
    void init_functionstructure();
    void init_partialcodeview();
...
Workbench::do_init (Gtk::Main &a_main)
{
    ...
    init_toolbar ();
    init_body ();
    init_functionstructure();
    init_partialcodeview();
    init_progressbar();
        ...
        for (plugin_iter = m_priv->plugin_manager->plugins_map ().begin ();
            ...
            if (plugin_iter->second && plugin_iter->second->entry_point_ptr ()) {
                ...
                if (perspective) {
                   ...
                   Gtk::Widget& a_widget=perspective->get_terminal_box ();
                    a_widget.show_all ();
                    m_priv->vtebox->pack_start(a_widget);

                    Gtk::Widget& b_widget=perspective->get_call_stack_box ();
                    b_widget.show_all ();
                    m_priv->function->pack_start(b_widget, Gtk::PACK_EXPAND_WIDGET);

                    Gtk::Widget& c_widget=perspective->get_source_view_widget2 ();
                    c_widget.show_all ();
                    m_priv->bodies2_container->pack_start(c_widget, Gtk::PACK_EXPAND_WIDGET);


                 //   Gtk::Widget& d_widget=perspective->get_progressbar_widget ();
                 //   d_widget.show_all ();
            //		m_priv->progressbar->pack_start(d_widget);
...
Gtk::Box *Workbench::get_progressbar ()
{
    CHECK_WB_INIT;
    THROW_IF_FAIL (m_priv && m_priv->progressbar);
...
Workbench::set_title_extension (const UString &a_str)
{
    ...
    if (a_str.empty ()) {
        ...
        //get_root_window ().set_title ("Visual Debugger");
    } else {
...
// If you want erase menubar, this section make reference
    //Gtk::Box *menu_container =
    //    ui_utils::get_widget_from_gtkbuilder<Gtk::Box> (m_priv->builder, "progressbar");
    //menu_container->pack_start (*m_priv->menubar);
    //menu_container->show_all ();

}
void
Workbench::init_vtebox ()
{
    LOG_FUNCTION_SCOPE_NORMAL_DD;

    m_priv->vtebox=ui_utils::get_widget_from_gtkbuilder<Gtk::Box> (m_priv->builder, "menucontainer");
    m_priv->vtebox->show_all ();
}
...
void
Workbench::init_functionstructure ()
{
    LOG_FUNCTION_SCOPE_NORMAL_DD;

    m_priv->function =
        ui_utils::get_widget_from_gtkbuilder<Gtk::Box> (m_priv->builder,
                                                             "functionstructure");
    m_priv->function->show_all ();
}

void
Workbench::init_partialcodeview ()
{
    LOG_FUNCTION_SCOPE_NORMAL_DD;
    m_priv->bodies2_container =
        ui_utils::get_widget_from_gtkbuilder<Gtk::Box> (m_priv->builder,
                                                             "partialcodeview");
    m_priv->bodies2_container->show_all ();

}

void
Workbench::init_progressbar ()
{
